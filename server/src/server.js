...
const HF_MODEL_ID = process.env.HF_MODEL_ID ?? 'meta-llama/Llama-3.2-1B-Instruct';
        ...
        const response = await fetch('https://router.huggingface.co/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a helpful assistant that only pulls answers from the provided document text. If the answer is not present, respond with "I do not know based on this document."',
                    },
                    {
                        role: 'user',
                        content: prompt,
...
app.post('/documents', upload.single('file'), async (req, res) => {
    try {
        ...
        if (isText) {
            ...
            if (!text.trim()) {
                console.warn(`No text extracted from PDF "${file.originalname}". It may be a scanned image or have unusual encoding.`);
            }
        }
...
function buildFallbackAnswer(doc, question) {
...
function selectRelevantSnippets(text, question) {
